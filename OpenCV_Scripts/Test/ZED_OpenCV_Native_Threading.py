...
# 
#'''
# Here we use Python threading for capture video (capturing can be bottleneck)
# You don't get any extra perfomance with this threading example (least when you capture directly from camera)
...
from Video_Threading import capture_video_class  # Thread classes
...
# For programming lovers: resolution set (switch)
...
# Parse resolution
height = switch_resolution(SET_RESOLUTION)[0]
width = switch_resolution(SET_RESOLUTION)[1]

# Create Threads from class and start it
capture_video = capture_video_class(CAMERA, height, width).start()
...
while True :
  ...
  captured_frame = capture_video.read()
...
# Stop the video capturing thread
capture_video.stop()
exit(0)
